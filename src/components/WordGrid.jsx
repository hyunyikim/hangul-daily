...
// Cell dimensions scale down as syllable count grows
const CELL_CFG = {
...
function JamoCell({ char, isSelected, isShaking, status, cfg, fullWidth, onClick }) {
  ...
  function borderStyle() {
    if (status !== 'current') return 'bg-violet-50 border-violet-200 border-solid';
    if (isShaking) return 'bg-red-50   border-red-400   border-solid';
    if (char) return 'bg-emerald-50 border-emerald-400 border-solid';
    if (isSelected) return 'bg-violet-100 border-violet-500 border-solid';
  ...
  return (
    <motion.button
      ...
      className={`rounded-xl border-2 flex items-center justify-center font-bold
        ${cfg.fontSize} text-slate-900 ${borderStyle()}
        ${status === 'current' ? 'cursor-pointer' : 'cursor-default'}`}
      style={{ width: w, height: h, fontFamily: 'var(--font-korean)', flexShrink: 0 }}>
      {char && (
        <motion.span
          key={char}
...
// ── One syllable's jamo grid — 3 layout types ───────────────────────────────
function SyllableGroup({ syl, status, jamo, vLayout, selectedSlot, shakingSlots, cfg, onSlotSelect }) {
  ...
  // ── vertical: 초성 / 중성 / (종성) stacked in one column ──────────────────
  if (vLayout === 'vertical') {
    return (
      <div className={`flex flex-col ${status === 'future' ? 'opacity-30' : ''}`} style={{ gap: GAP }}>
        {cell('초성')}
        {cell('중성')}
        {hasJong && cell('종성')}
      </div>
  ...
  // ── compound: [초성][중성_H] / [중성_V spanning] / ([종성 spanning]) ────────
  if (vLayout === 'compound') {
    return (
      <div className={`flex flex-col ${status === 'future' ? 'opacity-30' : ''}`} style={{ gap: GAP }}>
  ...
  // ── horizontal (default): [초성][중성] / ([종성 spanning]) ───────────────────
  return (
    <div className={`flex flex-col ${status === 'future' ? 'opacity-30' : ''}`} style={{ gap: GAP }}>
      <div className="flex" style={{ gap: GAP }}>
        {cell('초성')}
  ...
  return (
    <motion.div
      ...
      {word.syllables.map((syl, i) => {
        ...
        const vLayout = vowelLayout(syl.components['중성']);
        const compound = COMPOUND_VOWELS[syl.components['중성']];

        // jamo values per slot for this syllable
        const jamo = {
